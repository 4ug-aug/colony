...
test('POST /run appends the Grill turn contract so questions must use tools', async () => {
  ...
  expect(starts[0]!.task).toContain('workspace_set_grill_frontier')
  expect(starts[0]!.task).toContain(
    'Granted workspace tools are the three Grill tools plus read-only workspace_list_docs / workspace_get_doc',
  )
  sqlite.close()
})
