...
/** Shared truncation budget for step text and similar tool payloads. */
export const STEP_TEXT_LIMIT = 20_000
...
export function boundStepText(value: unknown): string {
  ...
  if (text.length <= STEP_TEXT_LIMIT) return text
  return `${text.slice(0, STEP_TEXT_LIMIT)}\n…[truncated]`
}
...
export function parseStep(line: string): Step | undefined {
  ...
  const kind = obj['kind']
  if (kind !== 'message' && kind !== 'tool_call' && kind !== 'tool_result')
    return undefined
